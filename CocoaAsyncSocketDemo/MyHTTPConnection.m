...
//	NSString* uploadDirPath = [[config documentRoot] stringByAppendingPathComponent:@"upload"];
    ...
    app.path = uploadDirPath;
    NSLog(@"--------------%@",uploadDirPath);
    NSDictionary *dict =[[NSDictionary alloc] initWithObjectsAndKeys:app.name,@"textOne",app.path,@"textTwo", nil];
    [[NSNotificationCenter defaultCenter]postNotificationName:@"reload" object:dict];
	BOOL isDir = YES;
